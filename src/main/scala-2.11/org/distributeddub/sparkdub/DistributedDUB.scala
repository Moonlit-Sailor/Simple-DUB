...
object execute {
  def main(args: Array[String]): Unit = {
    ...
    val pointsByPartition = points.mapPartitionsWithIndex( (partitionId, iter) => {
      val pointsArray = iter.toArray

      val localBoundarySet = broadcastDUB.value.phase1Search(k, r, T1, pointsArray, dim)
      val pointsAfterPhase1 = broadcastDUB.value.exclude(pointsArray,localBoundarySet,k)
      val phase2BoundPoints = broadcastDUB.value.phase2Search(pointsAfterPhase1,localBoundarySet,T1,T2,k,r)

      val result = pointsAfterPhase1.filter(
        p => phase2BoundPoints forall(b => broadcastDUB.value.sqdist(b.map(_.toDouble/k),p)>r*r))
      Seq((partitionId,result)).iterator
    })

    pointsByPartition.foreach(t => println(t._1 + ":" + t._2.length))
//    pointsByPartition.saveAsTextFile("hdfs://10.0.0.23:9000/resultPoints")
